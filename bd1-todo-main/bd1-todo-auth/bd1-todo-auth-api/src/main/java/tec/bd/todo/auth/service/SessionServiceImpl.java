...
public class SessionServiceImpl implements SessionService {
    ...
    public void addNewClient(ClientCredentials credentials) {
        // validar clientId y el clientSecret minimo 5 caracteres max 50.
        if(credentials.getClientId().length() < 5 || credentials.getClientId().length() > 50) {
            throw new CredentialsException("clientId must be between 5 and 50 characters");
        }

        // validar - el clientId ya existe en la BD? Lanzar excepcion
        if(this.clientRepository.findByClientId(credentials.getClientId()) != null) {
            throw new CredentialsException("clientId already exists");
        }
    ...
    public void deleteClient(String clientId) {
        // validar clientId y el clientSecret minimo 5 caracteres max 50. Sino lanzar excepcion
        if(clientId.length() < 5 || clientId.length() > 50) {
            throw new CredentialsException("clientId must be between 5 and 50 characters");
        }

        //validar si el clientId existe en la BD
        if(this.clientRepository.findByClientId(clientId) == null) {
            throw new CredentialsException("clientId does not exist");
        }
